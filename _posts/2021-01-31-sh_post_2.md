...
To approximate a function $f(\theta, \varphi)$ which maps the surface of the sphere to $[-1, 1]$, we can use an infinite sum of real spherical harmonics basis functions multiplied by some coefficients $f_\ell^m$:
